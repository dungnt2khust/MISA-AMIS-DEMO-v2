...
{
    public class VocativeRepository : BaseRepository<Vocative>, IVocativeRepository
    {
        #region Constructor
        public VocativeRepository(IConfiguration configuration) : base(configuration)
        {
        }
        #endregion
        ...
        public object GetVocativeFilterPaging(string searchData, int pageIndex, int pageSize)
        {
            using (_dbConnection = new NpgsqlConnection(_connectionString))
            {
                ...
                var sql = "select * from  public.func_get_vocative_paging_filter(@search_data) limit @page_size offset @offset;";
                sql += "select count(*) from (select * from  public.func_get_vocative_paging_filter(@search_data)) as filtertable;";
                ...
                var Vocatives = response.Read<Vocative>().ToList();
